...
export const exportToGithub = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const internalKey = process.env.TIBEBCRAFT_CONVEX_INTERNAL_KEY;
    if (!internalKey) {
      throw new NonRetriableError("TIBEBCRAFT_CONVEX_INTERNAL_KEY is not configured");
    };
    ...
    const { data: repo } = await step.run("create-repo", async () => {
      return await octokit.rest.repos.createForAuthenticatedUser({
        name: repoName,
        description: description || `Exported from Tibebcraft project ${projectId}`,
    ...
    const { data: commit } = await step.run("create-commit", async () => {
      return await octokit.rest.git.createCommit({
        owner: user.login,
        repo: repoName,
        message: "Initial commit from tibebcraft export",
        tree: tree.sha,
